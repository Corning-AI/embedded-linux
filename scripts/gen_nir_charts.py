...
# Data from "校准后冷刺激测试" (25°C start, 30 samples)
# ════════════════════════════════════════════
def chart_cold_stimulus_calibrated():
    ...
    ax1.axhline(-0.210, color=CYAN, linestyle='--', alpha=0.6, linewidth=1.5, label='Warm baseline (−0.210)')
    ax1.axhline(-0.12, color=RED, linestyle=':', alpha=0.5, linewidth=1.5, label='DANGER threshold (−0.12)')
    ax1.axhline(-0.15, color=YELLOW, linestyle=':', alpha=0.5, linewidth=1.5, label='WARNING threshold (−0.15)')

    # Fill danger/warning/safe zones
    ...
    l2 = ax2.plot(time_s, temp, 's--', color=ORANGE, linewidth=1.5,
                  markersize=6, alpha=0.7, label='Skin temp (°C)')

    ax1.set_xlabel('Time after cold removal (s)')
    ax1.set_ylabel('TOI_cal (white-reference calibrated)')
    ax2.set_ylabel('Skin Temperature (°C)', color=ORANGE)
    ax1.set_title('Calibrated Cold Stimulus Test — TOI_cal Recovery Curve (25°C start)')
    ...
    # Annotations
    ax1.annotate('Near-frostbite\n(hand barely tolerable)',
                 xy=(0, -0.118), xytext=(15, -0.10),
                 arrowprops=dict(arrowstyle='->', color=RED, lw=1.5),
                 fontsize=9, color=RED, ha='center')
    ax1.annotate('Reactive hyperemia\nbegins',
...
def chart_three_methods():
    ...
    ax1.set_title('Raw TOI\n(no correction)', color=RED)
    ax1.set_ylabel('TOI')
    ax1.set_xticks(x)
    ...
    ax2.set_title('White-ref TOI\n(LED bias removed)', color=YELLOW)
    ...
    ax3.set_title('DPF-corrected StO2\n(fingertip, physiological)', color=GREEN)
    ax3.set_ylabel('StO2')
    ax3.set_xticks(x)
    ax3.set_xticklabels(phases, fontsize=8)
    ax3.set_ylim(0, 0.55)
    ax3.axhline(0.43, color=RED, linestyle=':', alpha=0.5, label='DANGER')
    ax3.axhline(0.40, color=YELLOW, linestyle=':', alpha=0.5, label='WARNING')
    ax3.grid(True, axis='y')
    ax3.bar_label(bars3, fmt='%.2f', fontsize=8, color='#aaa')
    ax3.legend(fontsize=8)

    fig.suptitle('Three Correction Methods — Same Cold Stimulus Data', fontsize=15, color=WHITE, y=1.02)
    plt.tight_layout()
...
def chart_reproducibility():
    ...
    # Convergence band
    ax.axhspan(-0.21, -0.17, alpha=0.12, color=CYAN, label='Convergence band (−0.17 ~ −0.21)')
    ...
    ax.set_ylabel('Rewarming endpoint TOI_cal')
    ax.set_title('Five-Session Reproducibility — All endpoints converge to −0.17 ~ −0.21')
...
def chart_dpf_site_comparison():
    ...
    # Left: StO2 dynamic range comparison
    sites = ['Forearm\n(DPF ratio 1.11)', 'Fingertip\n(DPF ratio 1.20)']
    cold_sto2 = [0.39, 0.43]
    warm_sto2 = [0.36, 0.24]  # rewarming trough (reactive hyperemia)
    baseline_sto2 = [0.37, 0.38]

    x = np.arange(2)
    w = 0.2
    ax1.bar(x - w, cold_sto2, w, color=CYAN, alpha=0.8, label='Cold (vasoconstriction)')
    ax1.bar(x, baseline_sto2, w, color=GREEN, alpha=0.8, label='Warm baseline')
    ax1.bar(x + w, warm_sto2, w, color=ORANGE, alpha=0.8, label='Reactive hyperemia trough')

    ax1.set_xticks(x)
    ax1.set_xticklabels(sites, fontsize=10)
    ax1.set_ylabel('StO2')
    ax1.set_title('StO2 Dynamic Range by Measurement Site')
    ax1.set_ylim(0, 0.55)
    ...
    ax1.text(0.15, 0.375, '0.03\n(compressed!)', fontsize=9, color=RED, ha='left')

    ax1.annotate('', xy=(1, 0.24), xytext=(1, 0.43),
                 arrowprops=dict(arrowstyle='<->', color=GREEN, lw=2))
    ax1.text(1.15, 0.33, '0.19\n(wide range)', fontsize=9, color=GREEN, ha='left')

    # Right: DPF values by body site
    body_sites = ['Forehead', 'Temporal', 'Neonatal\nhead', 'Calf', 'Thigh',
                  'Forearm', 'Fingertip', 'Palm']
    dpf_ratios = [1.20, 1.22, 1.19, 1.16, 1.16, 1.16, 1.20, 1.15]
    colors = [PURPLE if r >= 1.19 else CYAN for r in dpf_ratios]
    colors[6] = GREEN  # highlight fingertip
    ...
    ax2.set_xlabel('DPF Ratio (680nm / 860nm)')
    ax2.set_title('DPF Ratio by Body Site\n(higher = better sensitivity)')
...
def chart_frostbite_threshold():
    # Countdown cold stimulus data (complete curve)
    samples = list(range(5, 30))
    # Interpolated from table data: cooling → peak cold → rewarming → baseline
    toi_cal_data = (
        [-0.12]*3 + [-0.12, -0.13, -0.13] +  # #5-#10 cooling
        [-0.14, -0.15, -0.155, -0.16, -0.165, -0.17] +  # #11-#16 peak cold
        [-0.17, -0.175, -0.18, -0.18, -0.185, -0.19, -0.19, -0.19] +  # #17-#24 rewarming
        [-0.19, -0.195, -0.195, -0.20, -0.20]  # #25-#29 near baseline
    )

    fig, ax = plt.subplots(figsize=(11, 6))

    # Warning zones
    ax.axhspan(-0.08, -0.12, alpha=0.15, color=RED, label='DANGER zone (> −0.12)')
    ax.axhspan(-0.12, -0.15, alpha=0.12, color=YELLOW, label='WARNING zone (−0.15 ~ −0.12)')
    ax.axhspan(-0.15, -0.25, alpha=0.08, color=GREEN, label='SAFE zone (< −0.15)')
    ...
    # Labels
    ax.text(7, -0.105, 'DANGER\nStop cooling!', fontsize=11, color=RED, ha='center', weight='bold')
    ax.text(13, -0.135, 'WARNING', fontsize=10, color=YELLOW, ha='center', weight='bold')
    ax.text(22, -0.22, 'SAFE', fontsize=10, color=GREEN, ha='center', weight='bold')
    ax.text(28, -0.215, 'baseline −0.210', fontsize=8, color=CYAN, ha='right')

    # Phase annotations
    ax.annotate('Severe\nvasoconstriction', xy=(6, -0.12), xytext=(3, -0.09),
                arrowprops=dict(arrowstyle='->', color=WHITE, lw=1),
                fontsize=8, color='#aaa', ha='center')
    ax.annotate('Reactive\nhyperemia', xy=(20, -0.19), xytext=(24, -0.16),
                arrowprops=dict(arrowstyle='->', color=WHITE, lw=1),
                fontsize=8, color='#aaa', ha='center')

    ax.set_xlabel('Sample # (2s intervals)')
    ax.set_ylabel('TOI_cal')
    ax.set_title('Frostbite Warning System — Three-Level Threshold Visualization')
